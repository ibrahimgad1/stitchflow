...
finishedInventoryRouter.get("/finished-inventory/movements", (req, res) => {
  ...
  const modelVariantId =
    typeof req.query.modelVariantId === "string"
      ? req.query.modelVariantId.trim()
      : "";
  if (modelVariantId) {
  ...
  const total = db
    .prepare(
      `SELECT COUNT(*) AS count FROM finished_stock_movements WHERE ${where}`,
    )
    .get(...values) as { count: number };
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Could not adjust finished stock";
...
finishedInventoryRouter.get("/model-variants/:id/movements", (req, res) => {
  ...
  const variant = db
    .prepare("SELECT id FROM model_variants WHERE id = ?")
    .get(req.params.id);
  if (!variant) {
    res
      .status(404)
      .json({ statusCode: 404, message: "Model variant not found" });
    return;
  }

  const total = db
    .prepare(
      "SELECT COUNT(*) AS count FROM finished_stock_movements WHERE model_variant_id = ?",
    )
