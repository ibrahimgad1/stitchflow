...
paymentMethodsRouter.get("/payment-methods", (req, res) => {
  ...
  const total = db
    .prepare(`SELECT COUNT(*) AS count FROM payment_methods WHERE ${where}`)
...
paymentMethodsRouter.post("/payment-methods", (req, res) => {
  const parsed = paymentMethodSchema.safeParse(req.body);

  if (!parsed.success) {
    res.status(400).json({ statusCode: 400, message: "Invalid payment method data" });
    return;
  }
  ...
  try {
    db.prepare(`
      ...
      VALUES (?, ?, ?)
    `).run(id, parsed.data.name, parsed.data.isActive === false ? 0 : 1);

    res.status(201).json({ id, ...parsed.data, isActive: parsed.data.isActive !== false });
  } catch {
    res.status(409).json({ statusCode: 409, message: "Payment method already exists" });
  }
});

paymentMethodsRouter.put("/payment-methods/:id", (req, res) => {
  const parsed = paymentMethodSchema.safeParse(req.body);
  ...
  try {
    ...
    if (result.changes === 0) {
      res.status(404).json({ statusCode: 404, message: "Payment method not found" });
      return;
    }
