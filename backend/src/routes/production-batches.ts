...
productionBatchesRouter.get("/production-batches/:id", (req, res) => {
  ...
  if (!batch) {
    res
      .status(404)
      .json({ statusCode: 404, message: "Production batch not found" });
    return;
  }
...
productionBatchesRouter.post(
  ...
  (req: AuthenticatedRequest, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({ statusCode: 400, message: "Invalid production batch data" });
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Could not create production batch";
...
productionBatchesRouter.post(
  ...
  (req: AuthenticatedRequest, res) => {
    ...
    try {
      startProductionBatch(db, String(req.params.id), parsed.data.startDate, req.user?.id);
      res.json({ id: req.params.id, status: "in_progress" });
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Could not start production batch";
      const statusCode = message.includes("not found") ? 404 : 400;
      res.status(statusCode).json({ statusCode, message });
    }
  },
...
productionBatchesRouter.post(
  ...
  (req: AuthenticatedRequest, res) => {
    ...
    if (!parsed.success) {
      res
        .status(400)
        .json({ statusCode: 400, message: "Invalid completion data" });
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Could not complete production batch";
...
productionBatchesRouter.post(
  ...
  (req: AuthenticatedRequest, res) => {
    ...
    try {
      cancelProductionBatch(db, String(req.params.id), req.user?.id);
      res.json({ id: req.params.id, status: "cancelled" });
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Could not cancel production batch";
      const statusCode = message.includes("not found") ? 404 : 400;
      res.status(statusCode).json({ statusCode, message });
    }
  },
...
const stageSchema = z.object({
  stage: z.enum(["draft", "cutting", "sewing", "finishing", "completed"]),
});
...
productionBatchesRouter.put(
  ...
  (req: AuthenticatedRequest, res) => {
    ...
    if (!parsed.success) {
      res.status(400).json({ statusCode: 400, message: "Invalid stage" });
      return;
    }
    const db = getDatabase();
    try {
      updateProductionStage(db, String(req.params.id), parsed.data.stage, req.user?.id);
      res.json({ id: req.params.id, stage: parsed.data.stage });
    } catch (error) {
      const message =
