...
export function createExpense(
...
): { id: string; expenseNumber: string; amountMinor: number; paymentStatus: string } {
  ...
  if (input.categoryId) {
    const category = db
      .prepare("SELECT id, is_overhead AS isOverhead FROM expense_categories WHERE id = ? AND is_active = 1")
      .get(input.categoryId) as { id: string; isOverhead: number } | undefined;
    if (!category) {
      throw new Error("Expense category not found");
    }
    if (input.overheadPeriodId && category.isOverhead !== 1) {
      throw new Error("Only overhead categories can be linked to an overhead period");
    }
  ...
  if (input.overheadPeriodId) {
    const period = db
      ...
      .get(input.overheadPeriodId) as { id: string; status: string } | undefined;
    if (!period) {
  ...
  runExpense();
  return { id: expenseId, expenseNumber, amountMinor, paymentStatus: input.paymentStatus };
}
...
export function createSafeTransfer(
  ...
  const fromSafe = db.prepare("SELECT id FROM safes WHERE id = ? AND is_active = 1").get(input.fromSafeId);
  if (!fromSafe) {
    throw new Error("Source safe not found");
  }

  const toSafe = db.prepare("SELECT id FROM safes WHERE id = ? AND is_active = 1").get(input.toSafeId);
  if (!toSafe) {
...
export function adjustSafeBalance(
  ...
  const runAdjustment = db.transaction(() => {
    ...
    `).run(
      ...
      JSON.stringify({ balanceMinor: newBalanceMinor, reason: input.reason.trim() })
    );
  });
...
export function createCapitalTransaction(
  ...
  if (input.ownerId) {
    const owner = db.prepare("SELECT id FROM owners WHERE id = ? AND is_active = 1").get(input.ownerId);
    if (!owner) {
  ...
  const description =
    input.transactionType === "capital_injection" ? "Capital injection" : "Owner withdrawal";
  ...
  runTransaction();
  return { id: transactionId, amountMinor, transactionType: input.transactionType };
}
